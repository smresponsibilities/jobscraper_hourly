...
pastes a prompt in by hand, or Codex drives it directly via tmux inside
...
volume rather than judgment — it doesn't need Codex's own tokens spent on
it. Route it to freebuff. Keep for Codex anything where getting it right
...
- **No memory of this conversation.** Every prompt must be fully
  ...
  write "continue from where you left off" or reference anything only Codex
  Code has seen.
...
5. **The scope boundary, stated explicitly every time**: for research, that
   ...
   Codex's job. For repo work, that it must stay on the worktree branch
...
Codex can start and feed freebuff itself instead of handing the user a
...
- Update HANDOFF.md's "in progress" / research-state section so the next
  session (Codex or freebuff) knows what's resolved and doesn't duplicate
  work.
